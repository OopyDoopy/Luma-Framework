...
{
   return float3(dot(float3(0.25, 0.5, 0.25), c), dot(float2(0.5, -0.5), c.xz), dot(float3(-0.25, 0.5, -0.25), c));
}

float3 FromOpponent(float3 o)
{
   return float3(dot(float3(1.0, 1.0, -1.0), o), // R = Y + C1 - C2
                 o.x + o.z,                      // G = Y + C2
...
void main(float4 v0: SV_Position0, float2 v1: TEXCOORD0, out float4 o0: SV_Target0)
{
   ...
   float2 bloomUv = v1.xy; // sampling with jitter offset seems to cause fireflies in bloom.
