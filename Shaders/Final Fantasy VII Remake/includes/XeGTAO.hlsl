...
#define XE_GTAO_ENABLE_DENOISE 0
#endif
...
{
    // FF7 REMAKE SPECIFIC: Exact Assembly Logic
    // r2.z = r0.z * cb1[57].x + cb1[57].y;
    // r2.w = r0.z * cb1[57].z + -cb1[57].w;
    // r2.w = rcp(r2.w);
    // r3.z = r2.z + r2.w;
    
    // We access the global cb1 buffer directly.
    float term1 = screenDepth * cb1[57].x + cb1[57].y;
    float term2 = 1.0f / (screenDepth * cb1[57].z - cb1[57].w);
    return term1 + term2;
}
    ...
    SamplerState depthSampler, out float packedEdges )
{
    ...
    // CHANGED: Convert to Linear View Space Depth immediately
    lpfloat4 valuesUL;
    valuesUL.x = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesUL_Raw.x, consts);
    valuesUL.y = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesUL_Raw.y, consts);
    valuesUL.z = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesUL_Raw.z, consts);
    valuesUL.w = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesUL_Raw.w, consts);

    // CHANGED: Gather Raw Depth for BR
    float4 valuesBR_Raw = sourceViewspaceDepth.GatherRed( depthSampler, float2( min((pixCoord + 1.0) * consts.ViewportPixelSize, consts.SampleUVClamp) ) ); 
    
    // CHANGED: Convert to Linear View Space Depth immediately
    lpfloat4 valuesBR;
    valuesBR.x = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesBR_Raw.x, consts);
    valuesBR.y = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesBR_Raw.y, consts);
    valuesBR.z = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesBR_Raw.z, consts);
    valuesBR.w = XeGTAO_ScreenSpaceToViewSpaceDepth(valuesBR_Raw.w, consts);
    ...
    const lpfloat pixRZ = valuesBR_Raw.z;
    const lpfloat pixBZ = valuesBR_Raw.x;
    ...
    // FIX: UE4 uses Reversed Depth (0.0 = Far/Sky).
    // If depth is 0 (Sky), return "No Occlusion".
    // XeGTAO returns (1.0 - visibility) in .w.
    // So for No Occlusion (visibility=1), we must return 0 in .w.
    if (valuesUL_Raw.y <= 0.000001) // Check for Far Plane (0.0)
    {
        return float4(0,0,0,0); // .w = 0 means Full Visibility
    }
#endif
...
#else
    // FIX: Removed Prey-specific dynamic radius scaling (remap/RadiusScalingMinDepth).
    // Reverted to standard XeGTAO logic: Radius = Base * Multiplier.
    ...
    // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
    {
        ...
        for( lpfloat slice = 0; slice < sliceCount; slice++ )
        {
            ...
            for( lpfloat step = 0; step < stepsPerSlice; step++ )
            {
                ...
                // FIX: Sample Raw Depth (.x) and Convert to Linear View Space
                // We use sourceViewspaceScaledDepth but treat it as the raw depth buffer (t2)
                float rawDepth0 = sourceViewspaceScaledDepth.SampleLevel( depthSampler, min(sampleScreenPos0 * consts.RenderResolutionScale, consts.SampleScaledUVClamp), mipLevel ).x;
                float SZ0 = XeGTAO_ScreenSpaceToViewSpaceDepth(rawDepth0, consts);
                ...
                // FIX: Sample Raw Depth (.x) and Convert to Linear View Space
                float rawDepth1 = sourceViewspaceScaledDepth.SampleLevel( depthSampler, min(sampleScreenPos1 * consts.RenderResolutionScale, consts.SampleScaledUVClamp), mipLevel ).x;
                float SZ1 = XeGTAO_ScreenSpaceToViewSpaceDepth(rawDepth1, consts);
